<template>
  <view class="container">
    ...
    <view class="cate-content dis-flex" v-if="merchantList.length > 0">
      ...
      <scroll-view class="cate-left f-28" :scroll-y="true" :style="{ height: `${scrollHeight}px` }">
          <view v-for="(item, index) in merchantList" :key="index">
              ...
              <view class="type-nav" :class="{ selected: curIndex == index }" @click="handleSelectNav(index)">
                  <image class="logo" lazy-load :lazy-load-margin="0" v-if="item.mtMerchant.logo" :src="item.mtMerchant.logo"></image>
                  <view class="name">{{ item.mtMerchant.name }}</view>
              </view>
          </view>
      </scroll-view>
      ...
      <scroll-view class="cate-right b-f" :scroll-top="scrollTop" :scroll-y="true" :style="{ height: `${scrollHeight}px` }">
        <view v-if="merchantList[curIndex]">
          <view class="cate-right-cont">
            <view class="cate-two-box">
              <view v-if="merchantList[curIndex].storeDtos.length" class="cate-cont-box">
                <view class="flex-five item" v-for="(item, idx) in merchantList[curIndex].storeDtos" :key="idx" @click="onTargetGoods(item.id)">
                  <view class="cate-img">
                    <image v-if="item.logo" lazy-load :lazy-load-margin="0" :src="item.logo"></image>
                  </view>
                  <view class="cate-info">
                    <view class="base">
                       <text class="name text">{{ item.name }}</text>
                       <text class="stock text">{{ item.description }}</text>
                       <text class="stock text">{{ item.phone }}</text>
                       <text class="stock text">{{ item.address }}</text>
                  ...
                  <!-- <view class="" v-for="(itemAct, index) in item.ActivityList" :key="index">
                    
                  </view> -->
                </view>
              </view>
              <empty v-if="!merchantList[curIndex].storeDtos.length" :isLoading="isLoading" tips="暂无商品~"></empty>
            </view>
          </view>
        </view>
      </scroll-view>
    ...
    onShow() {
      ...
      settingApi.storeList()
        .then(result => {
          ...
          console.log("category storeList == ", result);
      })

      GoodsApi.merchantList().then(result => {
        app.merchantList = result.data.dataList.content;
        console.log("merchantList == ", result.data.dataList.content);
      })
