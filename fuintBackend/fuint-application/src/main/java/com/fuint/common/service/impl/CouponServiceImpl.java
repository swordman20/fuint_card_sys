...
public class CouponServiceImpl extends ServiceImpl<MtCouponMapper, MtCoupon> implements CouponService {
    ...
    /**
     * 根据店铺ID获取卡券列表,每个卡券的对应当前用户的信息
     * @param storeDto 查询参数
     * @throws BusinessCheckException
     * */
    public List<CouponDto> queryCouponListByStoreId(StoreDto storeDto, Integer userId) throws BusinessCheckException {
        String storeIds = storeDto.getId().toString();
        List<MtCoupon> dataList = mtCouponMapper.queryByStoreId(storeIds.toString());
        ...
        MtMerchant merchantInfo = merchantService.queryMerchantById(storeDto.getMerchantId());
        storeDto.setMerchantName(merchantInfo.getName());

        List<CouponDto> content = new ArrayList<>();
        String baseImage = settingService.getUploadBasePath();
        storeDto.setLogo(baseImage + storeDto.getLogo());
        storeDto.setMerchantLogo(baseImage + merchantInfo.getLogo());
        for (int i = 0; i < dataList.size(); i++) {
            CouponDto item = new CouponDto();
            BeanUtils.copyProperties(dataList.get(i), item);
            item.setIsReceive(false);
