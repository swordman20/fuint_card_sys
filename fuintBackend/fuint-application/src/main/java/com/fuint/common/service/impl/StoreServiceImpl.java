...
public class StoreServiceImpl extends ServiceImpl<MtStoreMapper, MtStore> implements StoreService {
    ...
    public MtStore saveStore(StoreDto storeDto) {
        ...
        if (StringUtil.isNotEmpty(storeDto.getImages())) {
            mtStore.setImages(storeDto.getImages());
        }
        ...
        if (storeDto.getDetailDescription() != null) {
            mtStore.setDetailDescription(storeDto.getDetailDescription());
        }
     ...
     * */
    @Override
    public List<StoreDto> queryByDistance(Integer merchantNo, String keyword, String latitude, String longitude, UserInfo mtUser) throws BusinessCheckException {
        List<StoreDto> dataList = new ArrayList<>();
        ...
        List<StoreDistanceBean> distanceList = mtStoreMapper.queryByDistance(merchantNo, keyword, latitude, longitude);
        Map<String, Object> param = new HashMap<>();
        param.put("status", StatusEnum.ENABLED.getKey());
