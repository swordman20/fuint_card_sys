...
public class MerchantServiceImpl extends ServiceImpl<MtMerchantMapper, MtMerchant> implements MerchantService {
     ...
     */
    @Override
    public PaginationResponse<MerchantDto> clientQueryMerchantListByPagination(PaginationRequest paginationRequest, HttpServletRequest request) throws BusinessCheckException {
        Page<MtMerchant> pageHelper = PageHelper.startPage(paginationRequest.getCurrentPage(), paginationRequest.getPageSize());
        LambdaQueryWrapper<MtMerchant> lambdaQueryWrapper = Wrappers.lambdaQuery();
        lambdaQueryWrapper.ne(MtMerchant::getStatus, StatusEnum.DISABLE.getKey());
        ...
        for (MtMerchant merchant : dataList) {
            merchant.setLogo(imagePath + merchant.getLogo());
            List<StoreDto> storeList = storeService.queryByDistance(merchant.getId(), keyword, latitude, longitude, mtUser);

            MerchantDto item = new MerchantDto();
            item.setMtMerchant(merchant);
            item.setStoreDtos(storeList);

            merchantDtos.add(item);
        }
        ...
        paginationResponse.setTotalPages(pageHelper.getPages());
        paginationResponse.setTotalElements(pageHelper.getTotal());
        paginationResponse.setContent(merchantDtos);
